...
(defn data-table [tbl]
  (let [tbl-head (table-keys tbl)
        tbl-body (next tbl)]
    (map #(data-record tbl-head %) tbl-body)))
...
(defn str-field-to-int [field rec]
  (let [field-val-int (Integer/parseInt (rec field))]
    (assoc rec field field-val-int)))
...
;;
;; Hint: reduce, conj, merge, filter, map
